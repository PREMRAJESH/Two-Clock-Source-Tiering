...
│   ├── amber_rows_review.csv       <- TRACKED: 27 AMBER rows, 17y/10n (Wayback-verified, export-bug fixed)
...
| Item | Status | Detail |
|---|---|---|
| **27 AMBER precision-audit rows** | **COMPLETE (manual)** | Extracted to `data_derived/amber_rows_review.csv` (27 rows, entity breakdown: Apple Intelligence 13, Kimi 10, Qwen 2, Dream Machine 1, Lovable 1). Final: **17 `y`, 10 `n`** (export-bug fixed; full Wayback verification 2026-09-17). |
...
| AMBER rows extraction | **COMPLETE** (27 rows to amber_rows_review.csv; 17y/10n after full Wayback verification; export-bug fixed `extract_amber_rows.py`) |
...
1. **Sign-off on the AMBER-batch reconciliation** -- AMBER CSV is correct (17y/10n, export-bug fixed, full Wayback verification). Her confirmation of the 10 Kimi n-calls and the lifehacker y-call is still open.
