...
│   ├── amber_rows_review.csv       <- TRACKED: 27 AMBER rows, labeled 17y/10n
...
| Item | Status | Detail |
|---|---|---|
| **27 AMBER precision-audit rows** | **COMPLETE (manual)** | Extracted to `data_derived/amber_rows_review.csv` (27 rows, entity breakdown: Apple Intelligence 13, Kimi 10, Qwen 2, Dream Machine 1, Lovable 1). Final distribution: **17 `y`, 10 `n`** (Wayback-verified 2026-09-09; see session log). |
...
| AMBER rows extraction | **COMPLETE** (27 rows to amber_rows_review.csv; labeled 17y/10n, committed `3b50d5b`) |
...
1. **Sign-off on the AMBER-batch reconciliation** (CSV-vs-master export bug, 5 Kimi y->n overrides) -- labeling itself is complete (17y/10n), but her review/confirmation of these specific decisions is still open.
