...
| Item | Status | Detail |
|---|---|---|
| **27 AMBER precision-audit rows** | **BLOCKED (manual)** | Manual relevance judgment rows; kept deliberately separate from the automated pipeline. Needs a human (jointly with Viveka) to finish. Still pending — not touched by the contrast batch work. |
| **263-row contrast-week precision batch** | **PENDING (manual)** | `data_derived/ct_artlist_contrast.csv` (22/22 entities, 263 rows, collected + archived 2026-08-18, commit `cd47959`). Needs the same manual `relevant` y/n labeling as the original audit file. Still pending — collection is done, labeling is not. |
...
| 22-entity contrast-week audit collection | **COMPLETE** (22/22, 263 rows; `_filename_entity` matching fix; archived `reference/contrast_collection_2026-08-18/`; committed `cd47959`) — collection only, labeling still pending above |
