...
| `amber_rows_review.csv` | **27 AMBER precision-audit rows** — extracted from `inputs_frozen/ct_artlist_LABELING.xlsx` (entity, window, date, title, domain, url, suggested_label, relevant). | `ct_artlist_LABELING.xlsx` (original audit file). | **Audit data** — the 27 `?`-suggested rows from the precision audit; 17y / 10n. | Tracked milestone (`3b50d5b`) |
